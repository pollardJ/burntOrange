...
TRAIN = "/home/ubuntu/train_all/"
EMOS = {"A": 0, "D": 1, "F": 2, "H": 3, "U": 4, "S": 5, "N": 6}
EM_ABBREV = {"Angry": "A", "Disgust": "D", "Fear": "F", "Happy": "H", "Unhappy": "U", "Surprise": "S", "Neutral": "N"}
CSV = "/home/ubuntu/four_cnn_fc_4096_rescale_happy_disgust.csv"
PARAM = "/home/ubuntu/four_cnn_fc_4096_rescale_happy_disgust.npz"
EPOCHS = 100
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.DenseLayer(lasagne.layers.dropout(network, p=0.5),
                                        num_units=4096, W=lasagne.init.GlorotUniform(),
...
if __name__ == "__main__":
    ...
    files, binary_emos = get_image_files_two_emos("Happy", "Disgust", TRAIN)

    X_tr, Y_tr, X_te, Y_te = load_image_data(files, perc_tr=0.8, im_func=rescale_image, emo_ints=binary_emos)

    # open the .csv file to write the model output to file as it goes through 
    # training epochs
    csv_file = open(CSV, 'w')
    ...
    # these are ways to set up weight decay/regularization
    # that can be turned on by uncommenting
    """
    ...
    for epoch in range(EPOCHS):
        ...
        # uncomment this if using the weight decay/regularization option above
        """
        if epoch != 0 and epoch % 4 == 0:
            eta.set_value(eta.get_value() * decay)
        """

        # train the model in batches of 200 images 
    ...
    print "\nSaving the parameter weights to a .npz file..."
    np.savez(PARAM, *lasagne.layers.get_all_param_values(emo_cnn))
